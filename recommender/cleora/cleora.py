...
class Cleora:
    ...
    def compute_embeddings(self):
        ...
        adjacency_matrix = nx.to_numpy_array(
            self.graph, nodelist=self.node_list, weight="weight"
        )
        epsilon = 1e-10
        ...
        return {
            self.index_to_node[i]: embeddings[i] for i in range(len(self.node_list))
        }

    def recommend_movies(self, user_id: int, top_n: int = 5, threshold: float = 0.5):
        ...
        sims = []
        for other_user in [
            n for n in self.node_list if n.startswith("U") and n != user_node
        ]:
        ...
        user_movies = set(nx.neighbors(self.graph, user_node))

        for similar_user in similar_users:
            similar_user_movies = set(nx.neighbors(self.graph, similar_user))
            recommended_movies.update(similar_user_movies - user_movies)

        return [self.node_to_index[movie] for movie in list(recommended_movies)[:top_n]]


def save_user_embeddings(embeddings: dict, filename: str = "user_embeddings.pkl"):
    """Save user embeddings to a file."""
    user_embeddings = {
        node: emb for node, emb in embeddings.items() if node.startswith("U")
    }
...
def get_watched_movies(session, user_id):
    ...
    from src.model.utils import Movie, Rating  # Ensure correct import paths
...
if __name__ == "__main__":
    ...
    for i in range(1, 2):
        ...
        recommended_movies_ids = cleora.recommend_movies(user_id=i, top_n=8)

        movies = session.query(Movie).all()
        recommended_movies = [
            str(movie.title) for movie in movies if movie.id in recommended_movies_ids
        ]
        ...
        if watched_movies or recommended_movies:
            ...
            watched_movies_padded = watched_movies + [("", "")] * (
                max_len - len(watched_movies)
            )
            recommended_movies_padded = recommended_movies + [""] * (
                max_len - len(recommended_movies)
            )

            print(
                "\n{:<40} {:<10} {:<40}".format(
                    "Watched Movie", "Rating", "Recommended Movie"
                )
            )
            print("-" * 90)

            for (watched, rating), recommended in zip(
                watched_movies_padded, recommended_movies_padded
            ):
